const Clock = (function () {
  ...
  const progressInfo = document
    .querySelector(".custom-progress")
  ...
  function timer(seconds) {
    ...
    intervalID = setInterval(() => {
      ...
      if (leftTime <= 0) {
        ...
        playAlarm();
        displayControls();
        if (isStudy) tomatoes.setTomatoes(tomatoes.getTomatoes() + 1);
        clearInterval(intervalID);
      }
  ...
  function handleControl() {
    ...
    const errors = [
      {
        validate: isStudy && name === "study",
        message: '⚠️ "Study" is the current mode.',
      },
      {
        validate: isRest && name === "rest",
        message: `⚠️ "Rest" is the current mode.`,
      },
      {
        validate: leftTime === 0 && name === "playpause",
        message: "⚠️ Change the mode to keep going !",
      },
    ];
    const error = errors.find((error) =>
      error.validate ? error.message : false
    );
    if (error) {
    ...
    // Condition
    if (name === "playpause") {
      if (isPause) clearInterval(intervalID);
      else timer(leftTime);
    }
  ...
  function displayControls() {
    ...
    isMuted
      ? (toggleMuted.textContent = "🔕")
      : (toggleMuted.textContent = "🔔");
    isStudy
      ? studyMode.classList.add("disabled")
      : studyMode.classList.remove("disabled");
    isRest
      ? restMode.classList.add("disabled")
      : restMode.classList.remove("disabled");
  }
  ...
  function displayTime(currentSeconds) {
    ...
    timerDisplay.textContent = `${minutes >= 10 ? minutes : "0" + minutes}:${
...
const TodoList = (function () {
  ...
  function renderItems() {
    itemList.innerHTML = items
      .map((item) => {
        const check = `<i class="${
          item.done ? "far fa-check-circle circle" : "far fa-circle circle"
        }"></i>`;
        const expand = `<i class="${
          item.expand
            ? "fas fa-chevron-up expand"
            : "fas fa-chevron-down expand"
        }"></i>`;
        const tomatoes = `<span class="tomatoes">${
          item.tomatoes > 0 ? "🍅".repeat(item.tomatoes) : "🏆 Congrats "
        }</span>`;
        const control = `
        ...
        return `<div class="item">
                ...
                <div class="${
                  item.expand
                    ? "item-down custom-rwo show"
                    : "item-down custom-rwo"
                }">
                    ${tomatoes}
                    ${control}
                </div>
  ...
  function deleteItem(e) {
    ...
    const contentTarget =
      e.target.parentNode.parentNode.querySelector(".item-up");
    const item = items.find(
      (item) => item.text === contentTarget.textContent.trim()
    );
  ...
  function handleTomatoes(e) {
    if (
      !e.target.classList.contains("tomatoes") &&
    ...
    e.target.classList.contains("plus")
      ? (item.tomatoes += 1)
      : (item.tomatoes -= 1);
    if (item.tomatoes > 8) item.tomatoes = 8;
...
const Pages = (function () {
  ...
  function changePage(pageName) {
    pages.map((page) => {
      ...
      } else page.style.transform = "translateX(-150vw)";
    });
  }
